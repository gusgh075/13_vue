...
// axios : http client, 비동기 요창/응답을 쉽게 처리해주는 라이브러리
...
api.interceptors.response.use(
    ...
    async err => {  // 응답 실패
      ...
      // response에는 HTTP 응답 상태 코드가 담겨져 있다

      // 직전 요청이 토큰 재발급이였는데 401 === 재발급 실패
      ...
      if (response?.status === 401) {
        ...
        if (config._retry) {
          await authStore.clearAuth();  // 로그아웃 처리
          return Promise.reject(err); // 실패 처리
        }
        config._retry = true; // 다음 요청에서는 막도록
        ...
        try {
          ...
          return api(config); // 4. 재발급된 토큰으로 재요청, config에는 이전 기록이 있음
        } catch (refreshErr) {
          // 재발급 실패하면 로그아웃
          await authStore.clearAuth();
