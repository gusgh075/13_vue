...
export const useAuthStore = defineStore('auth', () => {
  ...
  function setAuth(at) {
    ...
    try {
      ...
      const payload = JSON.parse(atob(at.split('.')[1])); // .을 기준으로 JWT를 쪼갬 (HEADER, PAYLOAD, SIGNATURE)
      console.log('♥️♥️♥️♥️♥️payload♥️♥️♥️♥️♥️', payload);
  ...
  // Getters
  // access token이 존재하고, 만료일을 넘지 않으면 true, 아니면 false
  ...
  return {accessToken, userRole, expirationTime, setAuth, isAuthenticated, clearAuth};
});
